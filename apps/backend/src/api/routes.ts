...
export default async function routes(fastify: FastifyInstance) {
  ...
  fastify.post<{ Body: RunRequest }>('/query', {
    schema: {
    ...
    try {
      ...
      if (tool === "web-search") {
        ...
        const { text, totalTokenCount } = await generateFriendlyReply("search", prompt, results);
        ...
        const { text, totalTokenCount } = await generateFriendlyReply("calc", prompt, result);
